...
export function NumberFilter<TData>({ column, config }: NumberFilterProps<TData>) {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
        if (value === '' && operator !== 'isEmpty' && operator !== 'isNotEmpty' && operator !== 'between') {
             ...
             else column.setFilterValue({ operator, value: value ? Number(value) : undefined, value2: value2 ? Number(value2) : undefined });
        } else {
